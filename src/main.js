import React from 'react';

const categories = [
{ name: 'All', active: true },
{ name: 'Artificial Intelligence' },
{ name: 'Cloud Computing' },
{ name: 'DevOps' },
{ name: 'Programming Language' },
{ name: 'Mobile Application Development' },
{ name: 'Technology and Tools' },
{ name: 'Get a Job in IT Company' },
{ name: 'Others' },
];

const Main = () => {
return (

    <>
    <div  className=" filter-div " style={{ marginTop:'90px', backgroundColor: '#f2f6ff', marginLeft: 120, marginRight: 200 }}>
    <div className="category_container mt-0">Latest Posts</div>
    <div>
        <div className="filter-section">
        <div className="filter-section-icon-container">
            <div>
            <img
                className="filter-section-icon"
                src="data:image/png;base64,"
                alt="Filter icon"
            />
            </div>
            <p className="filter-section-text">Filter by Category</p>
        </div>
        <div className="category-list">
            {categories.map((category) => (
            <div
                key={category.name}
                className={`category-item ${category.active ? 'category-item-active' : ''}`}
            >
                {category.name}
            </div>
            ))}
        </div>
        </div>
    </div>
    </div>
    </>
);
};

export default Main;
